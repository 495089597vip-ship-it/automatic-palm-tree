...
def list_characters(db: Session = Depends(get_db)):
    rows = db.execute(text("SELECT id, name, description, appearance, personality, reference_image_url FROM characters ORDER BY id DESC")).mappings().all()
    return rows
...
def create_character(payload: CharacterCreate, db: Session = Depends(get_db)):
    row = db.execute(text("""
        INSERT INTO characters(name, description, appearance, personality, reference_image_url)
        VALUES(:name, :description, :appearance, :personality, :reference_image_url)
        RETURNING id, name, description, appearance, personality, reference_image_url
